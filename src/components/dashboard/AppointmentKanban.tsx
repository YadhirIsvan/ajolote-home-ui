...
const statusConfig: Record<Appointment["status"], { 
  label: string; 
  color: string; 
  bgColor: string; 
  borderColor: string;
...
const AppointmentKanban = ({ appointments, onStatusChange }: AppointmentKanbanProps) => {
  ...
  const [activeTab, setActiveTab] = useState<Appointment["status"]>("programada");
  ...
  const handleAdvanceStatus = () => {
    ...
    if (currentIndex < statusOrder.length - 1 && currentIndex < 3) {
      // Can only advance up to "completada"
  ...
  const handleRevertStatus = () => {
    ...
    if (currentIndex > 0) {
      ...
      onStatusChange(selectedAppointment.id, prevStatus);
    }
    setIsSheetOpen(false);
    setSelectedAppointment(null);
  };
  ...
  if (isMobile) {
    ...
    return (
      <div className="space-y-4">
        ...
        {/* Appointments List for Active Tab */}
        <div className="space-y-3 animate-fade-in">
          {currentAppointments.length === 0 ? (
            <EmptyState type="appointments" message={`No hay citas ${currentConfig.label.toLowerCase()}`} />
          ) : (
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-6 gap-4 min-h-[500px]">
        {statusOrder.map((status) => {
          const config = statusConfig[status];
          const columnAppointments = getAppointmentsByStatus(status);
          const Icon = config.icon;
...
const KanbanCard = ({ appointment, onClick, compact = false }: KanbanCardProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Status Badge - Only on non-compact */}
      {!compact && (
