...
const fetchPropertyFiles = async (propertyId: number): Promise<PropertyFileItem[]> => {
  const res = await fetch(`${API_BASE}/api/user/property-files/${propertyId}`);
  if (!res.ok) throw new Error("Error al cargar documentos");
  return res.json();
};

const uploadPropertyFiles = async (propertyId: number, files: File[]): Promise<void> => {
...
const ClientCompras = ({ onBack }: ClientComprasProps) => {
  ...
  const uploadMutation = useMutation({
    mutationFn: ({ propertyId, files }: { propertyId: number; files: File[] }) =>
      uploadPropertyFiles(propertyId, files),
    onSuccess: (_, { propertyId }) => {
      queryClient.invalidateQueries({ queryKey: ["property-files", propertyId] });
      queryClient.invalidateQueries({ queryKey: ["properties-buys"] });
    },
  });

  const displayList = comprasList.length > 1 ? comprasList : [];
  const viewingProperty = viewingDetailId ? comprasList.find((p) => p.id === viewingDetailId) : null;
  ...
  const stepsFromProgress = (progressStr: string, processStage: string): Step[] => {
    const pct = parseInt(progressStr?.replace(/%/g, "") || "0", 10);
    const stepLabels = ["Solicitud enviada", "Visita", "Documentos verificados", "Crédito aprobado", "Firma de contrato", "Entrega de llaves"];
    return stepLabels.map((label, i) => {
      const threshold = ((i + 1) / stepLabels.length) * 100;
      const done = pct >= threshold;
      const isDocStep = label === "Documentos verificados";
      return {
        label,
        done,
        current: !done && (i === 0 || stepLabels.slice(0, i).every((_, j) => pct >= ((j + 1) / stepLabels.length) * 100)),
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Back */}
      <button
        onClick={() => (viewingDetailId && comprasList.length > 1 ? setSelectedPropertyId(null) : onBack())}
      ...
      >
        ...
        {viewingDetailId && comprasList.length > 1 ? "Volver a lista" : "Volver al Dashboard"}
      </button>
      ...
      {/* When only 1 property: show detail directly. When >1: show cards or detail */}
      {comprasLoading ? (
        ...
        /* Single property: show detail directly */
        <PropertyDetailCard
          prop={singleProperty}
          files={filesData}
          filesLoading={filesLoading}
          steps={stepsFromProgress(singleProperty.overallProgress, singleProperty.processStage)}
          onUpload={() => triggerUpload(singleProperty.id)}
          uploadMutation={uploadMutation}
        />
      ) : (
        /* Multiple: show cards or selected detail */
        viewingDetailId && viewingProperty ? (
          <PropertyDetailCard
            prop={viewingProperty}
            files={filesData}
            filesLoading={filesLoading}
            steps={stepsFromProgress(viewingProperty.overallProgress, viewingProperty.processStage)}
                ...
                <CardContent className="p-0">
                  <div className="flex flex-col sm:flex-row">
                    <div className="sm:w-48 h-40 sm:h-auto bg-muted/30 relative flex-shrink-0">
                      <img src={prop.image || "/placeholder.svg"} alt={prop.title} className="w-full h-full object-cover" />
                      <Badge className="absolute top-3 left-3 bg-champagne-gold text-white text-xs">{prop.status}</Badge>
                    </div>
                    <div className="flex-1 p-5">
                      ...
                      <p className="text-sm text-foreground/60 mt-1">Agente: {prop.agent_name}</p>
...
interface PropertyDetailCardProps {
  prop: PropertyBuyItem;
  files: PropertyFileItem[];
  filesLoading: boolean;
  steps: Step[];
  onUpload: () => void;
  uploadMutation: { mutate: (vars: { propertyId: number; files: File[] }) => void; isPending: boolean };
}

const PropertyDetailCard = ({ prop, files, filesLoading, steps, onUpload, uploadMutation }: PropertyDetailCardProps) => {
          ...
          <div className="flex-1 p-6">
            ...
            <div className="mt-5">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-champagne-gold">{prop.overallProgress}</span>
              </div>
            ...
            {steps.map((step, i) => (
              <div key={step.label}>
                <div className="flex items-start gap-3 relative">
                  ...
                  <div
                    className={`${step.current && step.allowUpload && (files.length > 0 || filesLoading) ? "pb-2" : "pb-6"} ${step.current ? "font-semibold text-midnight" : step.done ? "text-foreground/70" : "text-foreground/40"}`}
                  >
                    ...
                    {filesLoading ? (
                      ...
                      files.map((f, fi) => (
                        ...
                        >
                          ...
                          <span className="text-foreground/40">({(f.size / 1024).toFixed(1)} KB)</span>
                        </div>
                      ))
                    ) : null}
                  </div>
