...
const ENDPOINT = "/public/sale-processes";
...
export const submitSellerLeadAction = async (
  ...
  const payload = {
    ...
    ...(membershipId !== undefined && { membership_id: membershipId }),
  };

  try {
    const { data: response } = await axiosInstance.post<BackendSellerLeadResponse>(
      ENDPOINT,
      payload
    );
    return {
